...
export default {
  ...
  data() {
    return {
      ...
      searchKey: '',//搜索Key
      addressList: [],//搜索结果
      mapKey: '你的key',//腾讯地图mapKey，需到https://lbs.qq.com/上申请
      map: null,
      markerLayer: null,
  ...
  methods: {
    ...
    handleSearch() {
      ...
      if (!this.searchKey) {
        ...
        this.timeout = setTimeout(() => {
          this.$apiUtil.baseQuery(`map/suggestion`,{keyword:this.searchKey,region:this.selfSelectedValue.city}).then(res => {
            console.log(res)
    ...
    _clickMap(evt){
      ...
      this.$apiUtil.baseQuery('map/geocoder',{location:locationParam}).then(res => {
